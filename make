...
linux32unamestr=`"${scriptdir}build/cmd/linux32/uname" 2> /dev/null`
linux64unamestr=`"${scriptdir}build/cmd/linux64/uname" 2> /dev/null`
osxunamestr=`"${scriptdir}build/cmd/osx/uname"  2> /dev/null`
win32unamestr=`"${scriptdir}build/cmd/win32/uname.exe" -o 2> /dev/null`

if [ "$linux32unamestr" == "Linux" ] || [ "$linux64unamestr" == "Linux" ]; then
# Linux
linuxuname64str=`"${scriptdir}build/cmd/linux64/uname" -m 2> /dev/null`
if [ "$linuxuname64str" == "x86_64" ]; then

# Linux64
#echo Host is Linux64
"${scriptdir}build/cmd/linux64/make" "$@" HOST_OS=Linux64

else

# Linux32
#echo Host is Linux32
"${scriptdir}build/cmd/linux32/make" "$@" HOST_OS=Linux32
...
#OSX
#echo Host is OSX
"${scriptdir}build/cmd/osx/make" "$@" HOST_OS=OSX

elif [ "${win32unamestr}" == "MinGW" ]; then

#MinGW / Cygwin
#echo Host is MinGW
"${scriptdir}build/cmd/win32/make.exe" "$@" HOST_OS=Win32
