...
static void _ota_fetch(void* user_data, int is_fetch, uint32_t size_file, char *purl, char *version)
{
    ...
    /* start */
    if (0 == is_fetch) {
        iotx_cmp_ota_parameter_t ota_parameter = {0};
        ...
        if (cmp_pt->ota_func)
            cmp_pt->ota_func(cmp_pt, &ota_parameter, cmp_pt->ota_user_context);
    } else {
        g_cmp_ota_status = 1;
    }
