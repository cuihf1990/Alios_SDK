...
int IOT_CMP_OTA_Start(char* cur_version, iotx_cmp_ota_handle_func_fpt ota_func, void* user_context, void* option)
{
    ...
    if (NULL == cur_version || NULL == ota_func) {
        CMP_ERR(cmp_log_error_parameter);
        return FAIL_RETURN;
    }

    if (g_cmp_cnt->ota_func) {
    ...
    if (NULL == g_cmp_cnt->ota_handler) {
        g_cmp_cnt->ota_func = NULL;
        g_cmp_cnt->ota_user_context = NULL;
        return FAIL_RETURN;
    }

    g_cmp_cnt->ota_func = ota_func;
    g_cmp_cnt->ota_user_context = user_context;
