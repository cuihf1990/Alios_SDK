...
static pthread_info_t thread_allocate_info(const char *thread_name)
{
	...
	for (i = 0; i < THREAD_NUM; ++i) {
		...
		if (0 == strcmp(pthread_info->name, thread_name)) {
			...
			OS_ASSERT((TS_ZOMBIE == pthread_info->state), "must in ZOMBIE state when reallocate, thread name = %s\n", thread_info_pool[i].name);
...
void *os_thread_create(const char *thread_name, void *thread_func, void *arg)
{
	...
	OS_ASSERT(NULL != thread_name && '\0' != *thread_name, "thread must have a name.\n");
	...
	if (0 != platform_thread_create(&thread, thread_name, thread_func, arg, stack, stack_size, &stack_used)) {
		goto do_error;
	}
