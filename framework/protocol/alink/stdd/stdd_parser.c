...
static int __get_id_from_hexstr(const char *hexstr, int str_len, void *id,
                                int32_t id_len)
{
    char fmt[16] = {0};
...
static void __free_endpoint_profile_list(dlist_t *endpoint_head)
{
    ...
    list_for_each_entry_safe_t(pos, next, endpoint_head, list_node,
                               endpoint_profile_t) {
        list_del(&pos->list_node);
...
void __dump_endpoint_profile(endpoint_profile_t *profile)
{
    ...
    list_for_each_entry_t(name_mapping, &profile->attr_head, list_node,
                          name_mapping_t) {
        /*只print名称不一相同的纪录*/
        if (strcmp(name_mapping->user_name, name_mapping->std_name) != 0) {
            os_printf("\t\taliasname:%s, stdname:%s\n", name_mapping->user_name,
                      name_mapping->std_name);
        }
    }

    os_printf("\tcmd mapping:\n");
    list_for_each_entry_t(name_mapping, &profile->cmd_head, list_node,
                          name_mapping_t) {
        if (strcmp(name_mapping->user_name, name_mapping->std_name) != 0) {
            os_printf("\t\taliasname:%s, stdname:%s\n", name_mapping->user_name,
                      name_mapping->std_name);
...
static int stdd_parse_element_cb(char *element_str, int str_len, int str_type,
                                 dlist_t *head);
static attr_element_t *stdd_parse_complix_type(char *element_str, int str_len,
                                               int value_type)
{
    ...
    list_for_each_entry_safe_t(pos, next, &sub_element_head, list_node,
                               data_list_t) {
        if (NULL ==  element) {
                                 ...
                                 dlist_t *element_head)//dlist_t *head
{
    ...
    /*mask code*/
    char *mask_code = json_get_value_by_name(element_str, str_len,
                                             PROFILE_KEY_MASK_CODE, &len, NULL);
    if (mask_code) {
        ret = __get_id_from_hexstr(mask_code, len, (uint8_t *)&element->mask_code,
                                   sizeof(element->mask_code));
    ...
    if (type->type_class == ALINK_DATATYPE_CLASS_STRUCTURE ||
        ...
        /*sub element str*/
        int value_type = 0;
        sub_element = json_get_value_by_name(element_str, str_len, PROFILE_KEY_ELEMENT,
                                             &len, &value_type);
        ...
        PTR_GOTO(element->child_element, err, "parser sub element fail, elementstr:%s",
                 element_str);
    }
...
static int stdd_parse_attr_cb(char *attr_str, int str_len, int str_type,
                              dlist_t *profile_head, char *cluster_id)//dlist_t *head, cluster_id
{
    int len, ret = SERVICE_RESULT_ERR;
    char *attr_id, *attr_name, *data_type = NULL;

    /*new attri profile*/
    attr_profile_t *profile = (attr_profile_t *)stdd_new_buff(sizeof(
                                                                  attr_profile_t));
    PTR_RETURN(profile, ret, "pmalloc fail");
    memset(profile, 0, sizeof(attr_profile_t));

    /*cluster id*/
    ret = __get_id_from_hexstr(cluster_id, strlen(cluster_id), &profile->cluster_id,
                               sizeof(profile->cluster_id));
    RET_GOTO(ret, err, "invalid cluster id:%s", cluster_id);

    /*attr id*/
    attr_id = json_get_value_by_name(attr_str, str_len, PROFILE_KEY_ATTR_ID, &len,
                                     NULL);
    PTR_GOTO(attr_id, err, "get attr id fail, attrstr:%s", attr_str);
    ret = __get_id_from_hexstr(attr_id, len, &profile->attribute_id,
                               sizeof(profile->attribute_id));
    RET_GOTO(ret, err, "invalid attr id:%s", attr_id);

    /*attr name*/
    attr_name = json_get_value_by_name(attr_str, str_len, PROFILE_KEY_ATTR_NAME,
                                       &len, NULL);
    ...
    /*data type*/
    data_type = json_get_value_by_name(attr_str, str_len, PROFILE_KEY_DATA_TYPE,
                                       &len, NULL);
    ...
    if (type->type_class == ALINK_DATATYPE_CLASS_STRUCTURE ||
        ...
        /*attr element*/
        int str_type = 0;
        char *element_str = json_get_value_by_name(attr_str, str_len,
                                                   PROFILE_KEY_ELEMENT, &len, &str_type);
    ...
    /*new list node*/
    data_list_t *attr_profile_node = (data_list_t *)stdd_new_buff(sizeof(
                                                                      data_list_t));
...
static int stdd_parse_cluster_attr_cb(char *cluster_str, int str_len,
                                      int str_type, dlist_t *profile_head)//dlist_t *head
{
    ...
    /*cluster id*/
    str_pos = json_get_value_by_name(cluster_str, str_len, PROFILE_KEY_CLUSTER_ID,
                                     &len, &type);
    ...
    /*attr set*/
    attr_set = json_get_value_by_name(cluster_str, str_len, PROFILE_KEY_ATTR_SET,
                                      &len, &type);
    PTR_RETURN(attr_set, SERVICE_RESULT_ERR, "get attr_set fail, cluster str:%s",
               cluster_str);
    if (type != JARRAY) {
...
static int stdd_parse_cmd_args_cb(char *cmd_arg_str, int str_len, int str_type,
                                  dlist_t *arg_head);
...
static int stdd_parse_cmd_args_cb(char *cmd_arg_str, int str_len, int str_type,
                                  dlist_t *arg_head)//dlist_t *cmd_arg_head
{
    int len, ret = SERVICE_RESULT_ERR;
    char *arg_name, *data_type = NULL;

    /*new cmd argument*/
    cmd_argument_t *cmd_arg = (cmd_argument_t *)stdd_new_buff(sizeof(
                                                                  cmd_argument_t));
    PTR_RETURN(cmd_arg, SERVICE_RESULT_ERR, "new cmd argument fail");
    memset(cmd_arg, 0, sizeof(cmd_argument_t));

    /*cmd name*/
    arg_name = json_get_value_by_name(cmd_arg_str, str_len, PROFILE_KEY_ARG_NAME,
                                      &len, NULL);
    ...
    /*data type*/
    data_type = json_get_value_by_name(cmd_arg_str, str_len, PROFILE_KEY_DATA_TYPE,
                                       &len, NULL);
    PTR_GOTO(data_type, err, "get data type fail, datatype:%s", data_type);
    data_type_t *type = stdd_get_datatype_by_name(data_type, len);
    PTR_GOTO(type, err, "invalid datatype:%s", data_type);
    ...
    /*mask code*/
    char *mask_code = json_get_value_by_name(cmd_arg_str, str_len,
                                             PROFILE_KEY_MASK_CODE, &len, NULL);
    if (mask_code != NULL) {
        ret = __get_id_from_hexstr(mask_code, len, (uint8_t *)&cmd_arg->mask_code,
                                   sizeof(cmd_arg->mask_code));
        RET_LOG(ret, "invalid mask code:%s", mask_code);
    }

    /*bitmap类型参数，继续解析子参数*/
    if (type->type_class == ALINK_DATATYPE_CLASS_BITMAP) {
        char *sub_argument = json_get_value_by_name(cmd_arg_str, str_len,
                                                    PROFILE_KEY_ELEMENT, &len, NULL);
...
static int stdd_parse_cmd_cb(char *cmd_str, int str_len, int str_type,
                             dlist_t *profile_head, char *cluster_id)//dlist_t *profile_head, cluster_id
{
    int len, ret = SERVICE_RESULT_ERR;
    ...
    /*cluster id*/
    ret = __get_id_from_hexstr(cluster_id, strlen(cluster_id),
                               (uint8_t *)&profile->cluster_id, sizeof(profile->cluster_id));
    RET_GOTO(ret, err, "invalid cluster id:%s", cluster_id);

    /*cmd id*/
    cmd_id = json_get_value_by_name(cmd_str, str_len, PROFILE_KEY_CMD_ID, &len,
                                    NULL);
    PTR_GOTO(cmd_id, err, "get cmd id fail, cmdstr:%s", cmd_id);
    ret = __get_id_from_hexstr(cmd_id, len, (uint8_t *)&profile->cmd_id,
                               sizeof(profile->cmd_id));
    RET_GOTO(ret, err, "invalid cmd id:%s", cmd_id);

    /*cmd name*/
    cmd_name = json_get_value_by_name(cmd_str, str_len, PROFILE_KEY_CMD_NAME, &len,
                                      NULL);
    ...
    /*direction*/
    char *direc = json_get_value_by_name(cmd_str, str_len, PROFILE_KEY_DIRECTION,
                                         &len, NULL);
    if (direc) {
        int direction = 0;
    ...
    /*arg list*/
    char *arg_list = json_get_value_by_name(cmd_str, str_len, PROFILE_KEY_ARG_LIST,
                                            &len, NULL);
    if (arg_list != NULL) {
...
static int stdd_parse_cluster_cmd_cb(char *cluster_cmd_str, int str_len,
                                     int str_type, dlist_t *profile_head)//dlist_t *profile_head
{
    int ret = SERVICE_RESULT_OK;
    ...
    /*cluster id*/
    str_pos = json_get_value_by_name(cluster_cmd_str, str_len,
                                     PROFILE_KEY_CLUSTER_ID, &len, &type);
    PTR_RETURN(str_pos, SERVICE_RESULT_ERR, "get clusterid fail");
    strncpy(cluster_id, str_pos,
            sizeof(cluster_id) > len ? len : sizeof(cluster_id) - 1);

    /*cmd set*/
    cmd_set = json_get_value_by_name(cluster_cmd_str, str_len, PROFILE_KEY_CMD_SET,
                                     &len, &type);
...
static int stdd_parse_str_cb(char *str, int str_len, int str_type,
                             dlist_t *str_head)
{
    int ret = SERVICE_RESULT_ERR;
...
static int stdd_parse_name_mapping_cb(char *mapping_str, int str_len,
                                      int str_type, dlist_t *mapping_head)//list_head
{
    int len, ret = SERVICE_RESULT_ERR;
    ...
    /*new name profile*/
    name_mapping_t *mapping = (name_mapping_t *)stdd_new_buff(sizeof(
                                                                  name_mapping_t));
    PTR_RETURN(mapping, ret, "pmalloc fail");

    /*std name*/
    std_name = json_get_value_by_name(mapping_str, str_len, PROFILE_KEY_STD_NAME,
                                      &len, NULL);
    PTR_GOTO(std_name, err, "get std name fail, mapping_str:%s, len = %d",
             mapping_str, str_len);
    strncpy(mapping->std_name, std_name,
            sizeof(mapping->std_name) > len ? len : sizeof(mapping->std_name) - 1);

    /*user name*/
    user_name = json_get_value_by_name(mapping_str, str_len, PROFILE_KEY_ALIAS_NAME,
                                       &len, NULL);
    PTR_GOTO(user_name, err, "get user name fail, mapping_str:%s", mapping_str);
    strncpy(mapping->user_name, user_name,
            sizeof(mapping->user_name) > len ? len : sizeof(mapping->user_name) - 1);
...
static int stdd_parse_endpoint_cb(char *endpoint_str, int str_len, int str_type,
                                  dlist_t *mapping_head)//endpoin_id, dlist_t
{
    int ret = SERVICE_RESULT_ERR;
    ...
    /*new attr profile*/
    endpoint_profile_t *profile = (endpoint_profile_t *)stdd_new_buff(sizeof(
                                                                          endpoint_profile_t));
    PTR_RETURN(profile, ret, "pmalloc fail");
    ...
    /*endpoint id*/
    endpoint_id = json_get_value_by_name(endpoint_str, str_len,
                                         PROFILE_KEY_ENDPOINT_ID, &len, &type);
    PTR_RETURN(endpoint_id, SERVICE_RESULT_ERR, "get endpoint id fail");
    ret = __get_id_from_hexstr(endpoint_id, len, (uint8_t *)&profile->endpoint_id,
                               sizeof(profile->endpoint_id));
    RET_GOTO(ret, err, "invalid endpoint id:%s", endpoint_id);

    /*attr name mapping*/
    mapping_set = json_get_value_by_name(endpoint_str, str_len,
                                         PROFILE_KEY_ATTR_MAPPING, &len, &type);
    if (mapping_set != NULL) {
        if (type != JARRAY) {
    ...
    /*cmd name mapping*/
    mapping_set = json_get_value_by_name(endpoint_str, str_len,
                                         PROFILE_KEY_CMD_MAPPING, &len, &type);
    if (mapping_set != NULL) {
        if (type != JARRAY) {
