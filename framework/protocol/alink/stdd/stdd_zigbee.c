...
static int __get_device_attr(unsigned char ieee_addr[8],
...
static int __set_device_attr(unsigned char ieee_addr[8],
                             unsigned char endpoint_id,
                             const char *attr_name, const char *attr_value)
{
    ...
    return ((zigbee_set_attr_cb_t)func)(ieee_addr, endpoint_id, attr_name,
                                        attr_value);
}
...
static int c_get_device_attr(lua_State *L)
{
    ...
    ret = __get_device_attr(devinfo->dev_base.u.ieee_addr,
                            (unsigned char)endpoint_id, attr_name);
...
static int c_set_device_attr(lua_State *L)
{
    ...
    log_trace("set attr: dev_id:%s, endpoint_id:%d, attr_name:%s, attr_value:%s",
              dev_id, endpoint_id,
    ...
    ret = __set_device_attr(devinfo->dev_base.u.ieee_addr,
                            (unsigned char)endpoint_id, attr_name, attr_value);
    RET_GOTO(ret, end, "set device attr fail, attr_name:%s, attr_value:%s",
             attr_name, attr_value);
...
static int c_report_device_attr(lua_State *L)
{
    ...
    RET_LOG(ret, "forward device attr fail, attr_name:%s, attr_value", attr_name,
            attr_value);
...
static int c_report_device_event(lua_State *L)
{
    ...
    RET_LOG(ret, "forward device event fail, attr_name:%s, attr_value", event_name,
            event_value);
...
static int c_exec_device_cmd(lua_State *L)
{
    ...
    log_trace("exec cmd: dev_id:%s, endpoint_id:%d, cmd_name:%s, cmd_args:%s",
              dev_id, endpoint_id,
              cmd_name, cmd_args);
    ...
    ret = __exec_device_cmd(devinfo->dev_base.u.ieee_addr,
                            (unsigned char)endpoint_id, cmd_name, cmd_args);
    RET_GOTO(ret, end, "excute device command fail, cmd_name:%s, cmd_args:%s",
             cmd_name, cmd_args);
                            ...
                            const char *attr_name[], const char *attr_value[])
{
    ...
    */
    if (devinfo->link_state == LINK_STATE_OFFLINE ||
    ...
    while (attr_name[i++]) {
        ...
        ret = stdd_report_custom_attr(devinfo->dev_base.dev_id,
                                      devinfo->dev_base.model_id, endpoint_id,
        ...
        ret = stdd_get_alink_attr_set(devinfo->dev_base.model_id, endpoint_id,
                                      attr_name[i - 1], attr_set, &array_size);
        ...
        if (array_size > 1) {
            log_warn("size of attribute mapping set:%d, attr_name:%s", array_size,
                     attr_name);
        ...
        ret = stdd_forward_device_status(devinfo,
...
int stdd_zbnet_get_attr(const char *devid_or_uuid, const char *attr_name)
{
    ...
    ret = stdd_get_custom_attr(devid_or_uuid, devinfo->dev_base.model_id,
                               attr_name);
    if (SERVICE_RESULT_OK == ret) {
    ...
    //3.云端属性名称转化为终端设备属性名称
    ret = stdd_get_attr_mapping_set(devinfo->dev_base.model_id, attr_name,
                                    &endpoint_id, attr_set, &array_size);
    if (ret != SERVICE_RESULT_OK || array_size == 0) {
    ...
    //4.调用协议栈接口获取第一个映射的属性
    ret = __get_device_attr(devinfo->dev_base.u.ieee_addr, endpoint_id,
                            array_size > 0 ? attr_set[0] : attr_name);
    RET_GOTO(ret, out, "get device attr fail, attr_name:%s",
             array_size > 0 ? attr_set[0] : attr_name);
                        ...
                        const char *attr_value)
{
    ...
    ret = stdd_set_custom_attr(devid_or_uuid, devinfo->dev_base.model_id, attr_name,
                               attr_value);
    if (SERVICE_RESULT_OK == ret) {
        log_trace("set custom attribute success, attr_name:%s, attr_value:%s",
                  attr_name, attr_value);
        goto out;
    }
    ...
    //异常情况:映射的属性数大于1的在lua脚本中处理
    if (array_size > 1) {
        log_warn("size of attribute mapping set:%d, attr_name:%s, mapping set:",
                 array_size, attr_name);
                        ...
                        const char *rpc_args)
{
    ...
    ret = stdd_exec_custom_cmd(devid_or_uuid, devinfo->dev_base.model_id, rpc_name,
                               rpc_args);
    if (SERVICE_RESULT_OK == ret) {
        log_trace("exec custom command success, cmd_name:%s, cmd_args", rpc_name,
                  rpc_args);
        goto out;
    }

    //3.用户rpc名称转zigbee命令名称
    ret = stdd_get_cmd_mapping_name(devinfo->dev_base.model_id, rpc_name,
                                    &endpoint_id,
                                    cmd_name, sizeof(cmd_name));
    if (ret != SERVICE_RESULT_OK) {
        log_warn("stdd_get_cmd_mapping_name fail, rpc_name:%s, rpc_args:%s", rpc_name,
                 rpc_args);
    }
                            ...
                            const char *event_name, const char *event_args)
{
    ...
    ret = stdd_report_custom_event(devinfo->dev_base.dev_id,
                                   devinfo->dev_base.model_id, endpoint_id,
                                   event_name, event_args);
    RET_GOTO(ret, out, "report custom event fail, cmd_name:%s, cmd_args:%s",
             event_name, event_args);

    log_trace("report custom event, cmd_name:%s, cmd_args:%s", event_name,
              event_args);
