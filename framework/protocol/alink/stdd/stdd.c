...
static int32_t __update_attr_profile(char *profile_str)
{
    ...
    list_for_each_entry_safe_t(n_pos, n_next, &new_list, list_node, data_list_t){
        ...
        list_for_each_entry_safe_t(pos, next, &attr_profile_head, list_node, data_list_t){
        ...
        profile[i++] = n_attr;
        if(i == array_size - 1){
            log_info("update attr profile to zigbee stack, size:%d, name:%s", i, n_attr->attr_name);
            //更新profile到zigbee模组端
            ret = ((zigbee_update_attr_profile_cb_t)alink_cb_func[_ALINK_ZIGBEE_UPDATE_ATTR_PROFILE])(profile);
...
static int32_t __update_cmd_profile(char *profile_str)
{
    ...
    list_for_each_entry_safe_t(n_pos, n_next, &new_list, list_node, data_list_t){
        ...
        list_for_each_entry_safe_t(pos, next, &cmd_profile_head, list_node, data_list_t){
        ...
        if(i == array_size - 1){
            ...
            //更新profile到zigbee模组端
            ret = ((zigbee_update_cmd_profile_cb_t)alink_cb_func[_ALINK_ZIGBEE_UPDATE_CMD_PROFILE])(profile);
...
static dev_profile_t *__stdd_load_device_profile(const char *profile_path, uint32_t model_id)
{
    int ret = SERVICE_RESULT_ERR;
    char *result = NULL;
...
int32_t stdd_update_global_profile(const char *profile_path)
{
    ...
    ret = __update_cmd_profile(result);
    RET_GOTO(ret, end, "update global cmd profile fail, profile path:%s", profile_path);
...
int32_t stdd_add_device_profile(uint8_t dev_type, uint32_t model_id, const char *profile_path)
{
    ...
    PTR_RETURN(dev_profile, SERVICE_RESULT_ERR, "load device profile fail, profile path:%s, model id:%d", profile_path, model_id);
...
int32_t stdd_update_device_profile(uint8_t dev_type, uint32_t model_id, const char *profile_path)
{
    ...
    dev_profile = __stdd_load_device_profile(profile_path, model_id);
    PTR_RETURN(dev_profile, SERVICE_RESULT_ERR, "load device profile fail, profile path:%s, model id:0x%08x", profile_path, model_id);
...
int32_t stdd_check_device_profile(uint8_t dev_type, uint32_t model_id)
{
    ...
    //load device profile
    ret = get_device_profile_file(dev_type, model_id, profile_path, sizeof(profile_path));
    ...
    char *name_array[], int *array_size)
{
    ...
    list_for_each_entry_t(endpoint, &dev_profile->endpoint_head, list_node, endpoint_profile_t){
        name_mapping_t *mapping;
        tmp_id = endpoint->endpoint_id;
        list_for_each_entry_t(mapping, &endpoint->attr_head, list_node, name_mapping_t){
    ...
    char *name_array[], int *array_size)
{
    ...
    list_for_each_entry_t(endpoint, &dev_profile->endpoint_head, list_node, endpoint_profile_t){
        ...
        name_mapping_t *mapping;
        list_for_each_entry_t(mapping, &endpoint->attr_head, list_node, name_mapping_t){
            if(strcmp(cmdname_or_attrname, mapping->std_name) == 0 && attr_count < *array_size){
...
int32_t stdd_get_custom_attr(const char *dev_id, uint32_t model_id, const char *attr_name)
{
    ...
    if(result[0] && strlen(result[0]) > 0){
        ...
        log_error("call lua function result is NULL, function:%s", LUA_FUNC_GET_CUSTOM_ATTR);
        ret = SERVICE_RESULT_ERR;
    }
            ...
            const char *attr_name, const char *attr_value)
{
    ...
    if(result[0] && strlen(result[0]) > 0){
        ...
        log_error("call lua function result is NULL, function:%s", LUA_FUNC_SET_CUSTOM_ATTR);
        ret = SERVICE_RESULT_ERR;
    }
            ...
            const char *cmd_name, const char *cmd_args)
{
    ...
    if(result[0] && strlen(result[0]) > 0){
        ...
        log_error("call lua function result is NULL, function:%s", LUA_FUNC_EXEC_CUSTOM_CMD);
        ret = SERVICE_RESULT_ERR;
    }
            ...
            const char *attr_name, const char *attr_value)
{
    ...
    if(result[0] && strlen(result[0]) > 0){
        ...
        log_error("call lua function result is NULL, function:%s", LUA_FUNC_REPORT_CUSTOM_ATTR);
        ret = SERVICE_RESULT_ERR;
    }
            ...
            const char *event_name, const char *event_args)
{
    ...
    RET_GOTO(ret, end, "call lua fuction \"%s\" fail", LUA_FUNC_REPORT_CUSTOM_EVENT);
    ...
    if(result[0] && strlen(result[0]) > 0){
        ...
        log_error("call lua function result is NULL, function:%s", LUA_FUNC_REPORT_CUSTOM_EVENT);
        ret = SERVICE_RESULT_ERR;
    }
...
int stdd_get_device_attr(const char *uuid, const char *attr_name, char *attr_value_buff, int buff_size)
{
    ...
    log_trace("get device attr, uuid:%s, attr_name:%s, attr_value:%s", uuid, attr_name, attr_value_buff);
    ...
    const char *attr_name, const char *attr_value)
{
    ...
    //1.post设备属性
    if(strlen(devinfo->dev_base.uuid) > 0 && devinfo->dev_base.dev_type != DEV_TYPE_WIFI){
...
int stdd_get_device_attrset(const char *devid_or_uuid, char *attrset_buff, int buff_size)
{
    ...
    PTR_GOTO(profile, out, "get device profile fail, model_id:0x%08x", (unsigned int)devinfo->dev_base.model_id);
...
const char *stdd_get_subdev_secret(uint32_t short_model, char *secret_buff, uint32_t buff_size)
{
    ...
    snprintf(file_path, sizeof(file_path) - 1, "%s%s/%s", os_get_storage_directory(), PROFILE_DIR_NAME, DEVICE_KEY_CONFIG_FILE_NAME);
    ...
    RET_GOTO(ret, out, "get %s failed from config file:%s", DEVICE_KEY_CONFIG_NAME, file_path);

    snprintf(shortmodel_str, sizeof(shortmodel_str), "%08x", short_model);
    model_config = json_get_value_by_name(result, strlen(result), shortmodel_str, &len, NULL);
    PTR_GOTO(model_config, out, "get devconfig fail, shortmodel:%s, keyconfig:", shortmodel_str, result);

    int secret_len = 0;
    char *secret_ptr = json_get_value_by_name(model_config, len, DEVICE_SECRET_NAME, &secret_len, NULL);
...
static int stdd_load_profile(void)
{
    ...
    if(ret == SERVICE_RESULT_OK){
        ...
        for(i  = 0; i < model_num; i++){
            profile_path[0] = '\0';
            ret = get_device_profile_file(DEV_TYPE_ZIGBEE, model_id[i], profile_path, sizeof(profile_path));
            RET_LOG(ret, "get device profile fail, devie model id:0x%08x", (unsigned int)model_id[i]);
            if(ret != SERVICE_RESULT_OK)
                continue;
            log_trace("model id:%08x, profile path:%s", (unsigned int)model_id[i],  profile_path);
            ret = stdd_update_device_profile(DEV_TYPE_ZIGBEE, model_id[i], profile_path);
            RET_LOG(ret, "update device profile fail, model id:0x%08x, profile:%s", (unsigned int)model_id[i], profile_path);
        }
    }
