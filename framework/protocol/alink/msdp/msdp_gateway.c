...
static int __set_attribute(const char *uuid, const char *attr_name, char *attr_value)
{
    ...
    log_trace("uuid = %s, attr_name = %s, attr_value = %s", uuid, attr_name, attr_value);
    ...
    if(attr_handler)
    {
        log_trace("Exist attribute(%s) handler on devtype:%02x", attr_name, helper->dev_type);
        if(helper->dev_type == DEV_TYPE_GATEWAY)
        {
...
static int msdp_get_status(char *params)
{
    ...
    attrset = json_get_value_by_name(params, strlen(params), JSON_KEY_ATTRSET, &attrset_len, NULL);
    if(attrset)
    ...
    if(NULL == attrset || attrset_size == 0){
        ...
        int uuid_len = 0;
        char *uuid_ptr = json_get_value_by_name(params, strlen(params), JSON_KEY_UUID, &uuid_len, NULL);
        PTR_RETURN(uuid_ptr, SERVICE_RESULT_ERR, CALL_FUCTION_FAILED, "json_get_value_by_name(uuid)");
    ...
    ret = msdp_get_device_status_handler(params, msdp_get_attr_each_cb, __get_attribute, &json_out);
    ...
    if(json_out){
        ...
        char devid_str[18] = {0};
        char *dev_id = json_get_value_by_name(params, strlen(params), JSON_KEY_DEVID, &str_len, NULL);
        if(dev_id != NULL){
...
static int msdp_rpc(char *params)
{
    ...
    json_ptr = json_get_value_by_name(params, strlen(params), JSON_KEY_SERVICE, &len, NULL);
    ...
    json_ptr = json_get_value_by_name(params, strlen(params), JSON_KEY_ARGS, &len, NULL);
    PTR_GOTO(json_ptr, end, CALL_FUCTION_FAILED, "json_get_value_by_name");
    ...
    char uuid[MAX_UUID_LEN] = {0};
    json_ptr = json_get_value_by_name(params, strlen(params), JSON_KEY_UUID, &len, NULL);
    PTR_GOTO(json_ptr, end, CALL_FUCTION_FAILED, "json_get_value_by_name");
...
int msdp_get_gw_attr(const char *gw_uuid, const char *attr_name, char *attr_value_buff, int buff_size)
{
    int ret = SERVICE_RESULT_ERR;
    char *output = NULL;
