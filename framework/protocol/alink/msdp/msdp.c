...
static void __dump_device_callback(const device_helper_ptr helper)
{
    ...
    for(i = 0; i < MAX_CALLBACK_NUM; i++){
        if(helper->option_callback[i] == NULL)
            continue;
        os_printf("\toption_callback[%d]:\"0x%08x\"\n", i, (unsigned long)helper->option_callback[i]);
    }
...
static int msdp_listener(int type, void *data, int dlen, void *result, int *rlen) {
    if(type == SERVICE_EVENT) {
        ...
        if (st == SERVICE_STATE_READY) {
         ...
         if(ACB_SET_DEVICE_STATUS_ARRAY == cmd_id){
            ...
            int len = 0;
            uuid_ptr = json_get_value_by_name(p->data, strlen(p->data), JSON_KEY_UUID, &len, NULL);
            if(NULL == uuid_ptr){
    ...
    void *pf_set)
{
    ...
    if(msdp_get_attr_handler(helper, name)){
        log_error("%s attribute already registered on devtype:%02x", name, helper->dev_type);
...
static void msdp_delete_service_cb(device_helper_ptr helper, const char *name)
{
    ...
    if(phandler){
        ...
        log_error("no exist %s service handler on devtype:%02x", name, helper->dev_type);
    }
    os_mutex_unlock(helper->mutex_lock);
...
int register_remote_service(char *uuid, const char *service[])
{
    ...
    if(NULL == service || NULL == service[0] || strlen(service[0]) == 0){
        ...
        count = msdp_get_all_rpcname(DEV_TYPE_GATEWAY, service_name, MAX_SERVICE_COUNT - 1);
        if(0 == count){
    ...
    snprintf(json_param, buff_size - 1, REGISTER_SERVICES_STRING_FMT, uuid, service_array);
...
int msdp_register_device_helper(uint8_t dev_type, dev_option_cb callback[MAX_CALLBACK_NUM])
{
...
int msdp_report_device_attrs(const char *uuid, const char *attrs[])
{
    ...
    if(len == sizeof(attr_set))
        return ret;
    snprintf(params, sizeof(params) - 1, GET_DEVICE_ATTR_STRING_FMT, uuid, attr_set);
...
int msdp_report_device_status(const char *uuid, const char *attr_name, const char *attr_value)
{
    ...
    buff_size = strlen(POST_ATTR_VALUE_STRING_FMT) + 64 + strlen(attr_name) + strlen(attr_value);
...
int msdp_set_device_status(const char *uuid, const char *attr_name, const char *attr_value)
{
    int ret = SERVICE_RESULT_ERR;
    char attr_set[64] = {0};
    ...
    if(attr_value[0] != '{' && attr_value[0] != '[')
        len = snprintf(attr_buff, sizeof(attr_buff) - 1, ATTR_NAME_VALUE_STRING_FMT, attr_name, attr_value, "");
    else/*value为json object*/
        len = snprintf(attr_buff, sizeof(attr_buff) - 1, ATTR_NAME_OBJECT_STRING_FMT, attr_name, attr_value, "");
    if(len >= sizeof(attr_buff) - 1)
        return ret;

    len = snprintf(params, sizeof(params) - 1, SET_DEVICE_ATTR_STRING_FMT, uuid, attr_set, attr_buff);
...
int msdp_request_remote_service(const char *uuid, const char *service_name, const char *args)
{
    int ret = SERVICE_RESULT_ERR;
    char params[256] = {0};
    int len = 0;

    len = snprintf(params, sizeof(params) - 1, REQUEST_SERVICE_STRING_FMT, uuid, service_name, args);
    if(len >= sizeof(params) - 1)
        return ret;
...
int msdp_dispatch_event(const char *devid_or_uuid, const char *method, char *params)
{
    int ret = SERVICE_RESULT_ERR;
    device_helper_ptr helper = NULL;
    uint8_t cmd_type = 0;

    log_trace("msdp dispatch event, uuid:%s, method:%s, parmas:%s", devid_or_uuid, method, params);
    ...
    void *pf_set)
{
    ...
    log_trace("register %s attribute handler success on devtype:%02x", name, helper->dev_type);
