...
const char SystemFormat[] = "{\"system\":{\"alink\":\"%s\",\"jsonrpc\":\"2.0\",\"lang\":\"en\",\"sign\":\"%s\",\"key\":\"%s\",\"time\":\"%s\"},";
const char RequestFormat[] = "\"request\":{\"cid\":\"%s\",\"rspID\":%d,\"target\":\"%s\",\"token\":\"%s\",\"uuid\":\"%s\"},";
const char RequestFormat_test[] = "\"request\":{\"cid\":\"%s\",\"rspID\":%d,\"service\":\"check\",\"target\":\"%s\",\"testId\":\"%s\",\"token\":\"%s\",\"uuid\":\"%s\"},";
#ifdef SUPPORT_ID2
const char SignatureFormat[] = "cid:%s,rspID:%d,target:%s,token:%s,uuid:%s,time:%s";
const char SignatureFormat_test[] = "cid:%s,rspID:%d,service:check,target:%s,testId:%s,token:%s,uuid:%s,time:%s";
#else
const char SignatureFormat[] = "cid:%s,rspID:%d,target:%s,token:%s,uuid:%s,time:%s,secret:%s";
const char SignatureFormat_test[] = "cid:%s,rspID:%d,service:check,target:%s,testId:%s,token:%s,uuid:%s,time:%s,secret:%s";
#endif
const char SyncTimeFormat[] = "\"method\":\"getAlinkTime\",\"params\":{},\"id\":%d}";
const char RegisterDeviceFormat[] = "\"method\":\"%s\",\"params\":{\"model\":\"%s\",\"sn\":\"%s\",\"mac\":\"%s\",\"version\":\"%s;APP2.0;OTA1.0\",\"description\":\"%s\"},\"id\":%d}";
const char UnregisterDeviceFormat[] = "\"method\":\"unregisterDevice\",\"params\":{\"model\":\"%s\",\"sn\":\"%s\",\"mac\":\"%s\"},\"id\":%d}";
const char LoginFormat[] =  "\"method\":\"loginDevice\",\"params\":{\"sdkversion\":\"%s\"},\"id\": %d}";
const char LoginFormatWithToken[] =  "\"method\":\"loginDevice\",\"params\":{\"sdkversion\":\"%s\", \"traceToken\": \"%s\"},\"id\": %d}";
const char LogoutFormat[] =  "\"method\":\"logoutDevice\",\"params\":{},\"id\": %d}";
const char PostFormat[] = "\"method\":\"%s\",\"params\":%s,\"id\":%d}";

const char registerByAuthTokenFormat[] = "\"method\": \"%s\",\"params\": {\"authToken\": \"%s\",\"name\": \"%s\",\"model\": \"%s\",\"type\": \"%s\",\"category\": \"%s\",\"sn\": \"%s\",\"mac\": \"%s\",\"manufacturer\": \"%s\",\"version\": \"%s\"},\"id\": %d";
...
static void alink_request_json_system(char *buf, int rspid, const char *target, char *time) {
    ...
    sprintf(buf, SystemFormat, main_device->info->alink_version, signature, devinfo_get_key(), time);
    if(main_device->config->test_mode) {
        sprintf(buf + strlen(buf), RequestFormat_test, main_device->info->cid, rspid, target, main_device->config->test_id, main_device->config->token, main_device->config->uuid);
    } else {
        sprintf(buf + strlen(buf), RequestFormat, main_device->info->cid, rspid, target, main_device->config->token, main_device->config->uuid);
    }
}
...
int32_t alink_sync_time(char *time, int len) {
    ...
    sprintf(uplink_buff+strlen(uplink_buff), SyncTimeFormat, alink_generate_trans_id());
...
int32_t alink_register(const device_t *dev) {
    ...
    if(!rsp) {
        ...
        alink_debug_protocol(MODULE_NAME_ALINK_PROTOCOL,"result: uuid %s, token %s", dev->config->uuid, dev->config->token);
        config_update();
    }
    put_protocol_buff();
    alink_error_code_handler(ret);
...
int32_t alink_logout(void) {
    ...
    get_protocol_buff();
    alink_request_json_system(uplink_buff, -1, "", "");
    sprintf(uplink_buff+strlen(uplink_buff), LogoutFormat, alink_generate_trans_id());
...
int32_t alink_response_get_x(char *in, char *out, int outLen, char *key) {
    ...
    if (!strcmp(key, "code")) {  //if "code" is required, just fulfill msg
        ...
        pVal = rsp.p_cData;
        len = rsp.dataLen;
    }else if (rsp.p_cData) {   //get value by key from "data", key is "uuid" or "time"
...
int32_t alink_parse_response_CB(char *p_cName, int iNameLen, char *p_cValue, int iValueLen, int iValueType, void *p_CBData) {
    ...
    if(!strncmp(p_cName, "result", iNameLen)){
        ...
        p_stRsp->p_cMsg = json_get_value_by_name(p_cValue, iValueLen, "msg", &p_stRsp->msgLen, 0);
        p_stRsp->p_cData = json_get_value_by_name(p_cValue, iValueLen, "data", &p_stRsp->dataLen, 0);
    }
    return JSON_PARSE_OK;
}

int32_t alink_parse_response(char *p_cJsonStr, int iStrLen, AlinkResponse *p_stRsp) {
    return json_parse_name_value(p_cJsonStr, iStrLen, alink_parse_response_CB, (void*)p_stRsp);
}

int32_t alink_parse_request_CB(char *p_cName, int iNameLen, char *p_cValue, int iValueLen, int iValueType, void *p_CBData) {
    int i;
    ...
    if(!strncmp(p_cName, "system", iNameLen)){
        p_stReq->p_cTime = json_get_value_by_name(p_cValue, iValueLen, "time", &p_stReq->timeLen, 0);
    }else if(!strncmp(p_cName, "request", iNameLen)){
        p_stReq->p_cAccount = json_get_value_by_name(p_cValue, iValueLen, "account", &p_stReq->accountLen, 0);
        if(main_device->config->test_mode) {  //test mode begin
            moktestid = json_get_value_by_name(p_cValue, iValueLen, "testId", &moktestidLen, 0);
...
int alink_error_code_handler(uint32_t code)
{
    switch (code) {
        ...
        case ALINK_CODE_ERROR_SUBDEV_NOT_LOGIN:
            LOGW(MODULE_NAME_ALINK_PROTOCOL,"subdev error(%d), devmgr will take care of it", code);
            break;
        //IGNORE
