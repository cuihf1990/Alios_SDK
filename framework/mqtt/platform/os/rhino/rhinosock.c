...
int32_t aliot_platform_tcp_write(uintptr_t fd, const char *buf, uint32_t len, uint32_t timeout_ms)
{
    ...
    t_end = aliot_platform_time_get_ms( ) + timeout_ms;
    len_sent = 0;
    err_code = 0;
    ret = 1; //send one time if timeout_ms is value 0

    do {
        t_left = aliot_platform_time_left(t_end, aliot_platform_time_get_ms( ));
    ...
    } while((len_sent < len) && (aliot_platform_time_left(t_end, aliot_platform_time_get_ms()) > 0));
