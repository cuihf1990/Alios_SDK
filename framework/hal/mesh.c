...
int hal_ur_mesh_get_bcast_chnlist(ur_mesh_hal_module_t *m, const uint8_t **chnlist)
{
    if (m == NULL)
        m = hal_ur_mesh_get_default_module();
...
int hal_ur_mesh_get_ucast_chnlist(ur_mesh_hal_module_t *m, const uint8_t **chnlist)
{
    if (m == NULL)
        m = hal_ur_mesh_get_default_module();
...
int hal_ur_mesh_set_meshnetid(ur_mesh_hal_module_t *m, const meshnetid_t *meshnetid)
{
    if (m == NULL)
        m = hal_ur_mesh_get_default_module();
...
int hal_ur_mesh_set_mac_address(ur_mesh_hal_module_t *m, const mac_address_t *addr)
{
    if (m == NULL)
        m = hal_ur_mesh_get_default_module();
...
int hal_ur_mesh_expire_key(struct ur_mesh_hal_module_s *m, uint8_t index)
{
    if (m == NULL)
        m = hal_ur_mesh_get_default_module();

    if ((m != NULL) && (m->ur_mesh_hal_expire_key != NULL))
        return m->ur_mesh_hal_expire_key(m, index);
