...
ifneq ($(DO_DOWNLOAD),)
-include ./out/$(SFLASHWRITER_TGT_DIR)/config.mk
endif

OUTPUT_DIR :=./out/$(FRAPP)/
...
ifneq ($(strip $(USE_DCT)),)
	...
	$(QUIET)$(PERL) -e "$(PERL_ESC_DOLLAR)x=-s'$(DCT)'; print \"\xff\" x $(PERL_ESC_DOLLAR)x;" > $(call CONV_SLASHES,./out/$(FRAPP)/blankDCT.bin)
	$(QUIET)$(CAT) $(call CONV_SLASHES,./out/$(FRAPP)/blankDCT.bin) >> $(OUTPUT_DIR_CONVD)sflash.bin
endif
ifneq ($(strip $(USE_OTA)),)
	$(QUIET)$(CAT) $(call CONV_SLASHES,./out/$(OTA_TGT_DIR)/binary/$(OTA_TGT_DIR).stripped.elf) >> $(OUTPUT_DIR_CONVD)sflash.bin
endif
	$(QUIET)$(ECHO_BLANK_LINE)
...
./out/$(SFLASHWRITER_TGT_DIR)/config.mk: sflash_writer_app
