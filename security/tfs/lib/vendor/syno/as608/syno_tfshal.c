...
{		
	...
	if (pkg_head->head_h != 0xEF && pkg_head->head_l != 0x01) {
		printf("SYNO TFSL:check head error!\n");
       		return ERROR_HEAD;
   	}

   	if (pkg_head->flag != 0x01) {
		printf("SYNO TFSL: check flag error!\n");
        	return ERROR_FLAG;
    	}
 	...
 	sum = calc_checksum(input + sizeof(struct package_head) , dlen);
  	...
  	if(sum != ((*pdata << 8) + *(pdata + 1))){
		printf("SYNO TFSL: input data checksum error\n");
		return ERROR_SUM;
	}
	...
	case 0x53:
		...
		if(len1 > 512) { printf("input data error ,too big length !\n");return ERROR_DATA;} 
...
#endif 
	default:
		printf("SYNO TFSL:input cmd error!\n");
		return ERROR_PARAM;
...
static int package_contexts(int cmd ,uint8_t *param,int param_len,uint8_t *contexts)
{
	...
	if(param == NULL || contexts == NULL){
		printf("SYNO TFSL:input memory error!\n");
		return -1;
	}
	/*head*/
	contexts[len++] = 0xef;
...
static int package_response_data(int cmd, uint8_t *output_data,uint8_t *output,uint32_t *out_len)
{
	...
	if(output_data == NULL || output == NULL || out_len == NULL){
		printf("SYNO TFSL:input memory error!\n");
		if(output_data == NULL)  printf("SYNO TFSL:output data null\n");
		if(output == NULL)	printf("SYNO TFSL:output null \n");
		if(out_len == NULL)	printf("SYNO TFSL:out len null\n");
		return -1;
	}
	...
	if(output_data[5] != cmd){
		printf("SYNO TFSL:cmd error:send cmd is 0x%02x  recive :0x%02x\n",cmd,output_data[5]);
		*out_len = 0;
		return -4;
	}
	...
	if(cmd != 0x54 && cmd != 0x01 && cmd != 0x02){  //cmd 0x54 have no data
		...
		if(data_len > (1024 - (sizeof(struct package_head) + 2))){
			printf("SYNO TFSL:data length error!data length is to big!\n");
			*out_len = 0;
			return -5;
		}
	...
	if(tmp != 0x00){ //ack error
		printf("SYNO TFSL:check ack data error![ack : 0x%02x]\n",tmp);
		*out_len = 0;
		return -6;
	}
	...
	if(cmd == 0x41)
		pkg_len = data_len + 3;
	else if(cmd==0x54||cmd==0x01||cmd==0x02)
		pkg_len = data_len + 2;
	else
		pkg_len = data_len + 4;
...
#ifdef CONFIG_DEBUG
	printf("report data: len %d\n",len);
	for(tmp = 0; tmp < len; tmp++)
		printf("0x%02x,",output[tmp]);	
	printf("\n\n");
#endif 	
...
int open_session(void **handle){
	...
	if(is_hwopend){
		printf("AS608 hardware id busying!\n");
		return DEVICE_BUSY;
	}
...
#endif 
	*out_len = 0;   		
	pdata =  (void *)malloc(length * sizeof(uint8_t) + 1);
	if(pdata == NULL){
		printf("SYNO TFSL:malloc pdata memory error!\n");
		return ERROR_MEM;
	}
	...
	memset(pdata,0,(length * sizeof(uint8_t) + 1));
    	...
    	if(ret != 0){
    		free(pdata);
    		printf("SYNO TFSL:unpack package error!\n");
    		return ret;
    	}
    
    	contexts_len = package_contexts(cmd,pdata,length,(void *)&contexts);
    	if(contexts_len < 0){
    		free(pdata);
    		printf("SYNO TFSL:package sending context error!\n");
    		return ERROR_PARAM;
    	}
    
	ret =  DeviceTransmit(handle,contexts,contexts_len,(void *)&output_data,&output_data_len);
	if(ret != 0){
		 free(pdata);
		 printf("SYNO TFSHAL:Hwardware transmit error!\n");
		 return ERROR_HWIO;
	}
#ifdef CONFIG_DEBUG
	printf("+++++++++++=hal recive len : %d+++++++++ \n",output_data_len);
	printf("recive data :\n");
	...
	if(ret != 0){
		free(pdata);
		printf("SYNO TFSL:package response data error!\n");
		return ERROR_PARAM;
	}
