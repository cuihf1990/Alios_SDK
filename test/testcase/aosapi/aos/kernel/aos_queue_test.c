...
static void CASE_aosapi_kernel_queue_param()
{
	...
	ret= yos_queue_recv(&queue, RHINO_WAIT_FOREVER, NULL, &recv_size);
	YUNIT_ASSERT_MSG(ret==RHINO_NULL_PTR, "ret=%d", ret);

	ret= yos_queue_recv(&queue, RHINO_WAIT_FOREVER, &recv_msg, NULL);
	YUNIT_ASSERT_MSG(ret==RHINO_NULL_PTR, "ret=%d", ret);

	yos_sem_new(&tmp_sem, 0);
	ret = yos_queue_recv((yos_queue_t*)&tmp_sem, RHINO_WAIT_FOREVER, &recv_msg, &recv_size);
	YUNIT_ASSERT_MSG(ret==RHINO_KOBJ_TYPE_ERR, "ret=%d", ret);
	yos_sem_free(&tmp_sem);
