...
void test_uradar_rsid_allocator_case(void)
{
    ...
    for(index = 1; index <= 11; index++) {
        ...
        YUNIT_ASSERT(UR_ERROR_NONE == rsid_allocate_sid(network, &node_id));
        YUNIT_ASSERT(index == node_id.sid);
    }
    YUNIT_ASSERT(11 == rsid_get_allocated_number(network));

    node_id.ueid[0] += 1;
    node_id.sid = 10;
    YUNIT_ASSERT(UR_ERROR_NONE == rsid_allocate_sid(network, &node_id));
    YUNIT_ASSERT(12 == node_id.sid);
    YUNIT_ASSERT(12 == rsid_get_allocated_number(network));
