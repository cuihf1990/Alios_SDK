...
YOC_SLIST_HEAD(g_networks_list);
YOC_SLIST_HEAD(g_hals_list);
    ...
    slist_for_each_entry(&g_hals_list, hal, hal_context_t, next) {
        ...
        if (is_wifi) {
            if (mm_get_mode() & MODE_SUPER) {
                ...
                network->flags |= NETWORK_PERMANENT_LEADER;
            } else {
                network = new_network_context(hal, index++, SID_ROUTER);
            }
        } else {
