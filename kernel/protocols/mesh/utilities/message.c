...
message_t *message_alloc(network_context_t *network, uint16_t length)
{
    ...
    message->dest = ur_mem_alloc(sizeof(mesh_dest_t));
    ...
    memset(message->dest, 0, sizeof(mesh_dest_t));
...
ur_error_t message_free(message_t *message)
{
    if (message != NULL) {
        ...
        pbuf_free((struct pbuf *)message->data);
        if (message->dest) {
            ur_mem_free(message->dest, sizeof(mesh_dest_t));
        }
        ur_mem_free(message, sizeof(message_t));
    }
...
ur_error_t message_copy(message_t *dest, const message_t *src)
{
    ...
    if (src->dest) {
        memcpy(dest->dest, src->dest, sizeof(mesh_dest_t));
    }
