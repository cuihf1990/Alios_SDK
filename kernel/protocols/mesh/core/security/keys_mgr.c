...
ur_error_t set_master_key(const uint8_t *key, uint8_t length)
{
    ...
    slist_for_each_entry(hals, hal, hal_context_t, next) {
        ...
        hal_ur_mesh_expire_key(hal->module, GROUP_KEY1_INDEX);
        hal_ur_mesh_activate_key(hal->module, MASTER_KEY_INDEX);
    }
...
ur_error_t set_group_key(uint8_t *payload, uint8_t length)
{
    ...
    slist_for_each_entry(hals, hal, hal_context_t, next) {
        ...
        hal_ur_mesh_expire_key(hal->module, MASTER_KEY_INDEX);
        hal_ur_mesh_activate_key(hal->module, GROUP_KEY1_INDEX);
    }
