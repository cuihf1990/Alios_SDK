...
static uint8_t ucast_addr_compress(ur_ip6_addr_t *ip6_addr, uint8_t *buffer, uint8_t *len)
{
    ...
    if (memcmp(prefix.addr, ip6_addr->m8, 8) != 0) {
        memcpy(buffer, &ip6_addr->m8[0], 16);
        *len = *len + 16;
        return UCAST_ADDR_128BIT;
    }

    if (memcmp(&prefix.addr[8], &ip6_addr->m8[8], 6) == 0) {
...
static uint8_t ucast_addr_decompress(uint8_t mode, mesh_ipv6_prefix_t *prefix, uint8_t *iphc_buffer,
                                     ur_ip6_addr_t *ip6_addr, uint16_t shortid)
{
    switch(mode) {
        case UCAST_ADDR_ELIDED:
            memcpy(&ip6_addr->m8[0], prefix->addr, 14);
            ip6_addr->m8[14] = (shortid >> 8) & 0xff;
            ip6_addr->m8[15] = shortid & 0xff;
            return 0;
            break;
        case UCAST_ADDR_16BIT:
            memcpy(&ip6_addr->m8[0], prefix->addr, 8);
...
static ur_error_t ipv6_header_decompress(iphc_header_t *iphc_header, mesh_header_t *mesh_header,
                                         uint8_t *buffer, uint8_t *hc_len)
{
    ...
    *(uint16_t *)iphc_header = ur_swap16(*(uint16_t *)iphc_header);
    ...
    offset += ucast_addr_decompress(iphc_header->SAM, &prefix, &iphc_data[offset],
                                    &ip6_header->src, mesh_header->src);
    ...
    if (iphc_header->M == MULTICAST_DESTINATION) {
        ...
        if (mesh_header->control_ext & MESH_HEADER_BCASTDESTSID) {
            ...
            header_destsid = (mesh_header_destsid_t *)((uint8_t *)mesh_header +
                                                       sizeof(mesh_header_t));
            dest_sid = header_destsid->dest;
        }
        offset += ucast_addr_decompress(iphc_header->DAM, &prefix, &iphc_data[offset],
                                        &ip6_header->dest, dest_sid);
    }
...
message_t* lp_header_decompress(message_t *message, mesh_header_t *mesh_header)
{
    ...
    error = ipv6_header_decompress(iphc_header, mesh_header, buffer, &iphc_len);
    ...
    dec_message = message_alloc(NULL, dec_header_len);
    if(dec_message == NULL) {
        message_free(message);
        ur_mem_free(buffer, (UR_IP6_HLEN + UR_UDP_HLEN));
        return NULL;
...
ur_error_t lp_reassemble(message_t *p, mesh_header_t *mesh_header,
                         message_t **reass_p)
{
    ...
    *((uint16_t *)frag_header) = ur_swap16(*(uint16_t*)frag_header);
    ...
    if (frag_header->dispatch == FRAG_1_DISPATCH) {
        ...
        while (lrh != NULL) {
            if (lrh->sender_addr == mesh_header->src) {
        ...
        ur_log(UR_LOG_LEVEL_DEBUG, UR_LOG_REGION_6LOWPAN,
               ...
               mesh_header->src, datagram_tag, datagram_size, message_get_msglen(p));
        ...
        lrh->sender_addr = mesh_header->src;
        ...
        if (message_get_msglen((message_t *)lrh->pbuf) > datagram_offset) {
            /* duplicate, ignore. */
            ur_log(UR_LOG_LEVEL_DEBUG, UR_LOG_REGION_6LOWPAN,
                   "lowpan6: received duplicated FRAG_N from"
                   " %04hx, tag=%u len=%u offset=%u, drop it\r\n",
                   mesh_header->src, datagram_tag, message_get_msglen(p), datagram_offset);
            return UR_ERROR_FAIL;
        } else if (message_get_msglen((message_t *)lrh->pbuf) < datagram_offset) {
        ...
        ur_log(UR_LOG_LEVEL_DEBUG, UR_LOG_REGION_6LOWPAN,
               ...
               mesh_header->src, datagram_tag, message_get_msglen(p),datagram_offset);
        message_concatenate((message_t *)lrh->pbuf, p, false);
        p = NULL;

        /* is packet now complete?*/
        if (message_get_msglen((message_t *)lrh->pbuf) >= lrh->datagram_size) {
            /* dequeue from reass list. */
            dequeue_list_element(lrh);

            /* get pbuf */
            *reass_p = (message_t *)lrh->pbuf;
