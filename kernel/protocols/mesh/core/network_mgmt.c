...
static void handle_discovery_timer(void *args)
{
    ...
    if (hal->discovery_channel >= hal->channel_list.num) {
        ...
        if (hal->discovery_result.meshnetid != BCAST_SID) {
            hal->discovery_times = hal->discovery_timeouts;
        }
        hal->discovery_times++;
    }
    ...
    if (hal->discovery_times < hal->discovery_timeouts) {
        ...
        nbr = get_neighbor_by_mac_addr(hal, hal->discovery_result.addr.addr);
                                    ...
                                    const mac_address_t *dest)
{
    ...
    if ((nbr = update_neighbor(src, tlvs, tlvs_length, true)) == NULL) {
        return UR_ERROR_FAIL;
    }
    ...
    send_discovery_response(network, &src->src);
    return error;
}
                                     ...
                                     const mac_address_t *dest)
{
    ...
    if (is_bcast_netid(netinfo->meshnetid)) {
        return UR_ERROR_NONE;
    }

    res = &network->hal->discovery_result;
    if (is_bcast_netid(res->meshnetid) ||
        res->meshnetid < get_main_netid(netinfo->meshnetid)) {
        memcpy(&res->addr, &src->src, sizeof(res->addr));
        res->channel = src->channel;
        if (mm_get_mode() & MODE_SUPER) {
            res->meshnetid = netinfo->meshnetid;
        } else {
            res->meshnetid = netinfo->sub_meshnetid;
        }
    }
