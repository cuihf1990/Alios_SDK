...
extern int yoc_schedule_call(void (*action)(void *), void *arg);
...
void process_ipaddr(int argc, char *argv[])
{
    ...
    ur_ip6_addr_t                ipaddr;

    if (argc == 0) {
        show_ipaddr(NULL);
    } else {
        if (strcmp(argv[0], "add") == 0) {
            string_to_ip6_addr(argv[1], &ipaddr);
            error = ur_mesh_add_addr(&ipaddr);
        } else if (strcmp(argv[0], "del") == 0) {
            string_to_ip6_addr(argv[1], &ipaddr);
            error = ur_mesh_del_addr(&ipaddr);
        }
    }
...
void process_nbrs(int argc, char *argv[])
{
    ...
    slist_for_each_entry(hals, hal, hal_context_t, next) {
        ...
        slist_for_each_entry(&hal->neighbors_list, nbr, neighbor_t, next) {
            ...
            response_append("," EXT_ADDR_FMT, EXT_ADDR_DATA(nbr->addr.addr));
            response_append(",0x%04x", nbr->netid);
            response_append(",0x%04x", nbr->sub_netid);
            response_append(",0x%04x", nbr->sid);

            response_append(",%d", nbr->stats.link_cost);
            response_append(",%d", nbr->subnet_size);
...
void process_stats(int argc, char *argv[])
{
    ...
    if (link_stats) {
        ...
        response_append("  bcast_sending %s\r\n", link_stats->bcast_sending ? "true" : "false");
        response_append("  ucast_sending %s\r\n", link_stats->ucast_sending ? "true" : "false");
        response_append("  bcast_sending_timeouts %d\r\n", link_stats->bcast_sending_timeouts);
        response_append("  ucast_sending_timeouts %d\r\n", link_stats->ucast_sending_timeouts);
    }
...
void process_sids(int argc, char *argv[])
{
    ...
    response_append("mobile:\r\n");
    nodes_list = get_ssid_nodes_list();
    slist_for_each_entry(nodes_list, mobile_node, sid_node_t, next) {
        if (is_partial_function_sid(mobile_node->sid)) {
            response_append(EXT_ADDR_FMT ", %04x\r\n", EXT_ADDR_DATA(mobile_node->ueid), mobile_node->sid);
        }
    }
}
...
void process_traceroute(int argc, char *argv[])
{
    ...
    if (ur_mesh_resolve_dest(&dest, &dest_sid) != UR_ERROR_NONE) {
        return;
    }
    send_trace_route_request(&dest_sid);
}
...
void ur_cli_cmd(char *buf, uint16_t length, cmd_cb_t cb, void *priv)
{
    ...
    yoc_schedule_call(do_cli, input_cli);
}
