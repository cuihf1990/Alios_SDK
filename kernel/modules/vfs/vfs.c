...
int vfs_init(void)
{
    ...
    if (yoc_mutex_new(&g_vfs_mutex) != 0) {
        return E_VFS_K_ERR;
    }
...
int yunos_open(const char *path, int flags)
{
    ...
    if (yoc_mutex_lock(g_vfs_mutex) != 0) {
        return E_VFS_K_ERR;
    }
    ...
    if (yoc_mutex_unlock(g_vfs_mutex) != 0) {
        return E_VFS_K_ERR;
    }
...
int yunos_poll(struct pollfd *fds, int nfds, int timeout)
{
    ...
    for (i = 0; i < nfds; i++) {
        ...
        pfd = &fds[n_i_fds];
        (node->ops->poll)(true, pfd, sem.hdl);
        n_i_fds ++;
    }
...
check_poll:
    ...
    for (j = 0; j < i; j++) {
        ...
        (node->ops->poll)(false, pfd, sem.hdl);
