...
static void timer_change_param_test(void)
{
    ...
    ret = yunos_timer_change(NULL, TIMER0_ROUND, TIMER0_ROUND);
    TIMER_VAL_CHK(ret == YUNOS_NULL_PTR);

    ret = yunos_timer_change(&timer_0_test, 0, TIMER0_ROUND);
...
static void task_timer0_entry(void *arg)
{
    ...
    while (1) {
        /* check yunos_timer_change param */
        ...
        ret = yunos_timer_create(&timer_1_test, "timer_1_test", timer_1_func,
                                 TIMER0_ROUND * 20, TIMER0_ROUND * 20, (void *)TIMER0_MAGIC, 0);
        TIMER_VAL_CHK(ret == YUNOS_SUCCESS);

        ret = yunos_timer_change(&timer_1_test, TIMER0_ROUND, TIMER0_ROUND);
        TIMER_VAL_CHK(ret == YUNOS_SUCCESS);

        ret = yunos_timer_start(&timer_1_test);
        TIMER_VAL_CHK(ret == YUNOS_SUCCESS);

        ret = yunos_timer_start(&timer_0_test);
        TIMER_VAL_CHK(ret == YUNOS_SUCCESS);
        ...
        ret = yunos_timer_stop(&timer_1_test);
        TIMER_VAL_CHK(ret == YUNOS_SUCCESS);

        ret = yunos_timer_del(&timer_1_test);
        TIMER_VAL_CHK(ret == YUNOS_SUCCESS);
