...
static uint8_t mm_region_break_case1(void)
{
    ...
    MYASSERT(check_free == MM_REGION_1_SIZE + MM_REGION_2_SIZE - r_num * sizeof(
                 k_mm_region_list_t));
    ...
    ret = yunos_mm_bf_alloc(&my_mm_region_list_head, (void **)&ptmp,
                            MM_REGION_TOTAL_SIZE, allocator);
    MYASSERT(ret == YUNOS_NO_MEM);

    ret = yunos_mm_bf_alloc(&my_mm_region_list_head, (void **)&ptmp,
                            MM_REGION_MAX_SIZE, allocator);
    MYASSERT(ret == YUNOS_NO_MEM);
    ...
    ret = yunos_mm_bf_alloc(&my_mm_region_list_head, (void **)&ptmp1, 60,
                            allocator);
    ...
    ret = yunos_mm_bf_alloc(&my_mm_region_list_head, (void **)&ptmp2,
                            free - MM_REGION_2_SIZE, allocator);
    ...
    MYASSERT(ret == YUNOS_SUCCESS);
    check_free = MM_REGION_1_SIZE + MM_REGION_2_SIZE - MM_REGION_VALID_REGION *
                 sizeof(k_mm_region_list_t);
    free = yunos_mm_region_get_free_size(&my_mm_region_list_head);
    MYASSERT(free == check_free);
    MYASSERT(my_mm_region_list_head.frag_num == --r_num);
    ...
    for (i = 1; i < 10; i++) {
        ret = yunos_mm_bf_alloc(&my_mm_region_list_head_2, &mp[i - 1], 4 * i + 2,
                                allocator);
        MYASSERT(ret == YUNOS_SUCCESS);
    }
    for (i = 1; i < 10; i++) {
    ...
    for (i = 1; i < 10; i++) {
        if (i % 2 == 0) {
            ret = yunos_mm_bf_alloc(&my_mm_region_list_head_2, &mp2[i - 1], 4 * i + 1,
                                    allocator);
            MYASSERT(ret == YUNOS_SUCCESS);
...
static uint8_t mm_region_break_case2(void)
{
    ...
    ret = yunos_mm_ff_alloc(&my_mm_region_list_head2, (void **)&ptmp,
                            MM_REGION_TOTAL_SIZE, allocator);
    MYASSERT(ret == YUNOS_NO_MEM);
    ...
    ret = yunos_mm_ff_alloc(&my_mm_region_list_head2, (void **)&ptmp, 48,
                            allocator);
    ...
    ret = yunos_mm_ff_alloc(&my_mm_region_list_head2, (void **)&ptmp1, 60,
                            allocator);
    MYASSERT(ret == YUNOS_SUCCESS);
    ...
    /* the first region is exhausted*/
    ret = yunos_mm_ff_alloc(&my_mm_region_list_head2, (void **)&ptmp2,
                            free - MM_REGION_2_SIZE, allocator);
    MYASSERT(ret == YUNOS_SUCCESS);
    check_free = MM_REGION_2_SIZE - sizeof(k_mm_region_list_t);
    free = yunos_mm_region_get_free_size(&my_mm_region_list_head2);
...
void mm_region_break_test(void)
{
    ...
    task_mm_region_entry_register(MODULE_NAME, (test_func_t *)mm_region_func_runner,
                                  sizeof(mm_region_func_runner) / sizeof(test_func_t));
