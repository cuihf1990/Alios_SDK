...
void ymod_init(void)
{
    ...
    memset( (void *)g_mod_info, 0x00,
            YUNOS_CONFIG_DLOAD_MOD_NUM * sizeof(mod_info_t));
...
int32_t ymod_load(int32_t image_fd)
{
    ...
    crc = crc16((unsigned char const *)(image_base + 4),
                sizeof(struct ymod_hdr) + header->text_len + header->data_len - 4);
    if (crc != header->crc) {
    ...
    data_ram_base = (uint32_t)dload_allocate_ram(header->data_len +
                                                 header->bss_len);
    ...
    /* data sectin mapping */
    map_inst = cmmu_map(image_data_base, data_ram_base,
                        header->data_len + header->bss_len);
    if (map_inst < 0) {
    ...
    *apiptr = &g_api_table;
    ...
    module_entry_t module_init = (module_entry_t)(image_base + sizeof(
                                                      struct ymod_hdr) + header->init_base);
    module_init();
...
int32_t ymod_unload(int32_t image_fd)
{
    ...
    module_entry_t module_deinit = (module_entry_t)(image_base + sizeof(
                                                        struct ymod_hdr) + header->deinit_base);
    module_deinit();

    /* free allocated ram size for this module */
    dload_free_ram((void *)g_mod_info[item].data_ram_base,
                   g_mod_info[item].data_size);
