...
kstat_t ringbuf_pop(k_ringbuf_t *p_ringbuf, void * pdata, size_t * plen)
{
    ...
    if (p_ringbuf->type == RINGBUF_TYPE_FIX) {
        ...
        else if ((*p_ringbuf->head & RINGBUF_LEN_MASK_TWOBIT) == RINGBUF_LEN_VLE_2BYTES) {
            /*length use 2 bytes*/
            len_bytes = 2;
        }
        else if ((*p_ringbuf->head & RINGBUF_LEN_MASK_TWOBIT) == RINGBUF_LEN_VLE_3BYTES) {
            /*length use 3 bytes*/
            len_bytes = 3;
        }
        else{
        ...
        *plen = len = ringbuf_headlen_decompress(len_bytes, c_len);
        ...
        if (p_ringbuf->head > p_ringbuf->tail && (split_len = p_ringbuf->end - p_ringbuf->head) < len) {
...
uint8_t ringbuf_is_full(k_ringbuf_t *p_ringbuf)
{
    ...
    if (p_ringbuf->type == RINGBUF_TYPE_FIX && p_ringbuf->freesize < p_ringbuf->blk_size) {
        return 1;
    }
