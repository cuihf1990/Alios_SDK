...
)
{
    ...
    if (cpu_id >= 0) {
        cpu_core_id = cpu_id;
    } else if  (ESPOS_TASK_NO_AFFINITY == cpu_id) {
        cpu_core_id = tskNO_AFFINITY;
    } else {
        return -EINVAL;
    }
    ...
    ret = xTaskCreatePinnedToCore(entry, name, stack_size, arg,
                                  prio, &os_task, cpu_core_id);
    if (ret == pdPASS) {
        ret = 0;
...
)
{
    ...
    *pname = pcTaskGetTaskName(os_task);
