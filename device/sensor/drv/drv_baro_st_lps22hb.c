...
static int drv_baro_st_lps22hb_validate_id(i2c_dev_t* drv,  uint8_t id_addr, uint8_t id_value)
{
    uint8_t value = 0x00;
    int ret = 0;
...
int drv_baro_st_lps22hb_init(void){
    ...
    ret = drv_baro_st_lps22hb_validate_id(&lps22hb_ctx, LPS22HB_WHO_AM_I_REG, LPS22HB_WHO_AM_I_VAL);
    if(unlikely(ret)){
        return -1;
    }
