...
/**
 * @file hal/mesh.h
 * @brief uRadar Mesh HAL definations for YOC
 * @detail mesh HAL defines mesh functionalities to manipulate radio chip including <BR>
 *         enable and disable mesh HAL, transmit and receive traffic, get and set configs of radio chip <BR>
 *         <BR>
 * @version since 5.5.0
 */
...
typedef struct ur_mesh_hal_module_s {
    ...
    int (*ur_mesh_hal_activate_key)(struct ur_mesh_hal_module_s *module,
                                    uint8_t index);
    int (*ur_mesh_hal_expire_key)(struct ur_mesh_hal_module_s *module,
                                  uint8_t index);
 ...
 *     Set media configuration result, 0 if success, -1 if fail
 */
int hal_ur_mesh_set_bcast_channel(ur_mesh_hal_module_t *module, uint8_t channel);
 ...
 *     Set media configuration result, 0 if success, -1 if fail
 */
int hal_ur_mesh_set_ucast_channel(ur_mesh_hal_module_t *module, uint8_t channel);
 ...
 *     Set media configuration result, 0 if success, -1 if fail
 */
int hal_ur_mesh_set_mac_address(ur_mesh_hal_module_t *module, const mac_address_t *addr);
...
/**
 * Expire the specified encryption key of HAL.
 *
 * @param[in] module The HAL module to be operated; if NULL, the default module will be operated
 * @param[in] index  The key index
 *
 * @return
 *     Expire result, 0 if success, -1 if fail
 */
int hal_ur_mesh_expire_key(struct ur_mesh_hal_module_s *module,
                               uint8_t index);
