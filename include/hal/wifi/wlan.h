...
enum wlan_sec_type_e{
   ...
   SECURITY_TYPE_AUTO,        /**< It is used when calling @ref hal_wlan_start_adv, read security type from scan result. */
};
...
/** 
 *  @brief  Input network paras, used in hal_wlan_start function.  
 */
...
/** 
 *  @brief  Input network precise paras in hal_wlan_start_adv function.  
 */
...
void hal_wlan_get_mac_address( uint8_t *mac );
 ...
 */
int hal_wlan_start(Network_InitTypeDef_st* inNetworkInitPara);
 ...
 */
int hal_wlan_start_adv(network_InitTypeDef_adv_st* inNetworkInitParaAdv);
 ...
 *             @arg Soft_AP: The soft AP that established by hal_wlan_start()
 *             @arg Station: The interface that connected to an access point
 *
 *  @return   kNoErr        : on success.
 *  @return   kGeneralErr   : if an error occurred
 */
int hal_wlan_get_ip_status(IPStatusTypedef *outNetpara, WlanInterfaceTypedef inInterface);
 ...
 *  @return   kGeneralErr   : if an error occurred
 */
int hal_wlan_get_link_status(LinkStatusTypeDef *outStatus);
 ...
 *          Register callback function using @ref mico_add_notification() before scan.
 */
void hal_wlan_start_scan_adv(void);
 ...
 *  @return   kGeneralErr   : if an error occurred
 */
int hal_wlan_suspend_station(void);
 ...
 *  @return   kGeneralErr   : if an error occurred
 */
int hal_wlan_suspend_softap(void);
/**
  * @}
  */
 ...
 *  @return   kGeneralErr   : if an error occurred
 */
int hal_wlan_start_easy_link(int inTimeout);
 ...
 *          Easylink APP.  
 *          hal_ sends a callback: mico_notify_EASYLINK_GET_EXTRA_DATA
 *          with function:
 *          void (*function)(int datalen, char*data, hal__Context_t * const inContext);
 *          that provide a buffer where the data is saved
 *
 *  @param  inTimeout: If easylink is executed longer than this parameter in backgound.
 *          hal_ stops EasyLink and sends a callback where nwkpara is NULL
 *
 *  @return   kNoErr        : on success.
 *  @return   kGeneralErr   : if an error occurred
 */

int hal_wlan_start_easy_link_plus(int inTimeout);
 ...
 *  @return   kGeneralErr   : if an error occurred
 */
int hal_wlan_stop_easy_link(void);
int hal_wlan_stop_easy_link_plus(void);
...
/** @defgroup YOS_WLAN_GROUP_3 MiCO Wlan WPS Functions
  * @brief Provide management APIs for WPS function in hal_.
  * @{
  */
/** @brief  Start WPS configuration procedure
 *
 *  @param  inTimeout: If WPS is executed longer than this parameter in backgound.
 *          hal_ stops WPS and sends a callback where nwkpara is NULL
 *
 *  @return   kNoErr        : on success.
 *  @return   kGeneralErr   : if an error occurred
 */
int hal_wlan_start_wps(int inTimeout);
...
/** @defgroup YOS_WLAN_GROUP_4 MiCO Wlan Airkiss Functions
  * @brief Provide management APIs for Airkiss function in hal_.
  * @{
  */
 ...
 *  @detail This function can read SSID, password and extra user data sent from
 *          Easylink APP.  
 *          hal_ sends a callback: mico_notify_EASYLINK_WPS_COMPLETED
 *          with function:
 *          void (*function)(network_InitTypeDef_st *nwkpara, hal__Context_t * const inContext);
 *          that provide SSID and password, nwkpara is NULL if timeout or get an error
 *
 *  @param  inTimeout: If airkiss is executed longer than this parameter in backgound.
 *          hal_ stops airkiss and sends a callback where nwkpara is NULL
 *
 *  @retval kNoErr.
 */
int hal_wlan_start_air_kiss(int inTimeout);

/** @brief  Stop wechat airkiss configuration procedure
 *  
 *  @retval kNoErr.
 */
int hal_wlan_stop_air_kiss(void);
/**
  * @}
  */

/** @defgroup YOS_WLAN_GROUP_5 MiCO Wlan Power Functions
  * @brief Provide management APIs for powersave function in hal_ wlan.
  * @{
  */
/** @brief  Enable IEEE power save mode
 * 
 *  @detail When this function is enabled, hal_ enter IEEE power save mode if
 *          hal_ is in station mode and has connected to an AP, and do not need 
 *          any other control from application. To save more power, use 
 *          @ref hal__mcu_powersave_config
 * @retval  None
 */

void hal_wlan_enable_power_save(void);
 ...
 * @retval  None
 */
void hal_wlan_disable_power_save(void); 
...
/** @defgroup YOS_WLAN_GROUP_6 MiCO Wlan debug output Functions
  * @brief Provide management APIs for wlan debug info output in hal_ wlan.
  * @{
  */
/**
 ...
 *
 */
void hal_wifimgr_debug_enable(int enable);
 ...
 */
int hal_wlan_monitor_rx_type(int type);
 ...
 *       
 */
int hal_wlan_start_monitor(void);

/** @brief  Stop wifi monitor mode
 * 
 */
int hal_wlan_stop_monitor(void);
 ...
 */
int hal_wlan_set_channel(int channel);
 ...
 */
void hal_wlan_register_monitor_cb(monitor_cb_t fn);
