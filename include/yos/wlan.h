...
#ifndef __YOSWLAN_H__
#define __YOSWLAN_H__
...
enum wlan_sec_type_e{
   ...
   SECURITY_TYPE_AUTO,        /**< It is used when calling @ref yos_wlan_start_adv, read security type from scan result. */
};
...
/** 
 *  @brief  Input network paras, used in yos_wlan_start function.  
 */
...
/** 
 *  @brief  Input network precise paras in yos_wlan_start_adv function.  
 */
...
/** @defgroup YOS_WLAN_GROUP_1 YOS Basic Wlan Functions
  * @brief Provide Basic APIs for YOS wlan functions
  * @{
  */

void yos_wlan_get_mac_address( uint8_t *mac );
 ...
 *          first scan all of the supported Wi-Fi channels to find a wlan that 
 ...
 *          disconnected after a successful connection, start the reconnection 
 ...
 */
int yos_wlan_start(Network_InitTypeDef_st* inNetworkInitPara);
 ...
 *          and fixed. If this fast connection is failed for some reason,  
 ...
 */
int yos_wlan_start_adv(network_InitTypeDef_adv_st* inNetworkInitParaAdv);
 ...
 *             @arg Soft_AP: The soft AP that established by yos_wlan_start()
 *             @arg Station: The interface that connected to an access point
 *
 *  @return   kNoErr        : on success.
 *  @return   kGeneralErr   : if an error occurred
 */
int yos_wlan_get_ip_status(IPStatusTypedef *outNetpara, WlanInterfaceTypedef inInterface);
 ...
 *  @return   kGeneralErr   : if an error occurred
 */
int yos_wlan_get_link_status(LinkStatusTypeDef *outStatus);

/** @brief  Start a wlan scanning in 2.4GHz in backfround.
 *  
 *  @detail Once the scan is completed, sends a notify: 
 ...
 *          Register callback function using @ref mico_add_notification() before scan.
 */
void yos_wlan_start_scan_adv(void);
 ...
 *  @return   kGeneralErr   : if an error occurred
 */
int yos_wlan_suspend_station(void);
 ...
 *  @return   kGeneralErr   : if an error occurred
 */
int yos_wlan_suspend_softap(void);
/**
  * @}
  */

/** @defgroup YOS_WLAN_GROUP_2 MiCO Wlan Easylink Functions
 ...
 *  @return   kGeneralErr   : if an error occurred
 */
int yos_wlan_start_easy_link(int inTimeout);
 ...
 *          Easylink APP.  
 *          yos_ sends a callback: mico_notify_EASYLINK_GET_EXTRA_DATA
 *          with function:
 *          void (*function)(int datalen, char*data, yos__Context_t * const inContext);
 *          that provide a buffer where the data is saved
 *
 *  @param  inTimeout: If easylink is executed longer than this parameter in backgound.
 *          yos_ stops EasyLink and sends a callback where nwkpara is NULL
 *
 *  @return   kNoErr        : on success.
 *  @return   kGeneralErr   : if an error occurred
 */

int yos_wlan_start_easy_link_plus(int inTimeout);
 ...
 *  @return   kGeneralErr   : if an error occurred
 */
int yos_wlan_stop_easy_link(void);
int yos_wlan_stop_easy_link_plus(void);
...
/** @defgroup YOS_WLAN_GROUP_3 MiCO Wlan WPS Functions
  * @brief Provide management APIs for WPS function in yos_.
  * @{
  */
/** @brief  Start WPS configuration procedure
 *
 *  @param  inTimeout: If WPS is executed longer than this parameter in backgound.
 *          yos_ stops WPS and sends a callback where nwkpara is NULL
 *
 *  @return   kNoErr        : on success.
 *  @return   kGeneralErr   : if an error occurred
 */
int yos_wlan_start_wps(int inTimeout);
...
/** @defgroup YOS_WLAN_GROUP_4 MiCO Wlan Airkiss Functions
  * @brief Provide management APIs for Airkiss function in yos_.
  * @{
  */
 ...
 *  @detail This function can read SSID, password and extra user data sent from
 *          Easylink APP.  
 *          yos_ sends a callback: mico_notify_EASYLINK_WPS_COMPLETED
 *          with function:
 *          void (*function)(network_InitTypeDef_st *nwkpara, yos__Context_t * const inContext);
 *          that provide SSID and password, nwkpara is NULL if timeout or get an error
 *
 *  @param  inTimeout: If airkiss is executed longer than this parameter in backgound.
 *          yos_ stops airkiss and sends a callback where nwkpara is NULL
 *
 *  @retval kNoErr.
 */
int yos_wlan_start_air_kiss(int inTimeout);

/** @brief  Stop wechat airkiss configuration procedure
 *  
 *  @retval kNoErr.
 */
int yos_wlan_stop_air_kiss(void);
/**
  * @}
  */

/** @defgroup YOS_WLAN_GROUP_5 MiCO Wlan Power Functions
  * @brief Provide management APIs for powersave function in yos_ wlan.
  * @{
  */
/** @brief  Enable IEEE power save mode
 * 
 *  @detail When this function is enabled, yos_ enter IEEE power save mode if
 *          yos_ is in station mode and has connected to an AP, and do not need 
 *          any other control from application. To save more power, use 
 *          @ref yos__mcu_powersave_config
 * @retval  None
 */

void yos_wlan_enable_power_save(void);
 ...
 * @retval  None
 */
void yos_wlan_disable_power_save(void); 

/**
  * @}
  */

/** @defgroup YOS_WLAN_GROUP_6 MiCO Wlan debug output Functions
  * @brief Provide management APIs for wlan debug info output in yos_ wlan.
  * @{
  */
/**
 ...
 *
 */
void yos_wifimgr_debug_enable(int enable);
 ...
 */
int yos_wlan_monitor_rx_type(int type);
 ...
 *       
 */
int yos_wlan_start_monitor(void);

/** @brief  Stop wifi monitor mode
 * 
 */
int yos_wlan_stop_monitor(void);
 ...
 */
int yos_wlan_set_channel(int channel);
 ...
 */
void yos_wlan_register_monitor_cb(monitor_cb_t fn);
