...
#define MOD_DIST_G_BW_N_LOW               (3)
#define MOD_DIST_G_BW_N_HIGH              (4)
...
const TXPWR_ST gtxpwr_tab_def_n[WLAN_2_4_G_CHANNEL_NUM] = {
    {7-3, 8, 1},  // ch1
    {8-3, 8, 1},
    {8-3, 8, 1},    
    {8-3, 8, 1},  // ch4
...
TXPWR_PTR gtxpwr_tab_n = NULL;
UINT32 g_tabn_use_def = 0;
...
static void manual_cal_start_fitting_txpwr(UINT32 rate)
{
    if((rate <= MAX_RATE_FOR_B_5M) || (rate == MAX_RATE_FOR_B)) {
        ...
        if(!gtxpwr_tab_n) {
            gtxpwr_tab_n = (TXPWR_PTR)os_malloc(sizeof(TXPWR_ST)*WLAN_2_4_G_CHANNEL_NUM); 
            if(!gtxpwr_tab_n){
                MCAL_WARN("start fitting, no memory for txpwr tab n\r\n");
                return;
            } 
            os_memset(gtxpwr_tab_n, 0, sizeof(TXPWR_ST)*WLAN_2_4_G_CHANNEL_NUM);
        }    
    }
}
...
int manual_cal_get_txpwr(UINT32 rate, UINT32 channel, UINT32 *mod, UINT32 *pa)
{
    ...
    if((!gtxpwr_tab_b) && (!gtxpwr_tab_g) && (!gtxpwr_tab_n)) {
...
static void manual_cal_adjust_fitting(TXPWR_PTR ptr, UINT8 dist)
{   
    if(ptr->valid == TXPWR_ELEM_UNUSED) 
        return;
    
    ptr->mod = (ptr->mod > dist)? ptr->mod - dist : 0;
}
...
static void manual_cal_do_fitting(TXPWR_PTR dst, TXPWR_PTR srclow, TXPWR_PTR srchigh)
{
    ...
    mod = (UINT8)((float)(srclow->mod + srchigh->mod) / 2 + 0.5);
    pa = (UINT8)((float)(srclow->pa + srchigh->pa) / 2 + 0.5); 
    dst->mod = mod;
    dst->pa = pa;
...
UINT32 manual_cal_fitting_txpwr_tab(void)
{
    ...
    if(!gtxpwr_tab_n) {
        gtxpwr_tab_n = (TXPWR_PTR)os_malloc(sizeof(TXPWR_ST)*WLAN_2_4_G_CHANNEL_NUM); 
        if(!gtxpwr_tab_n){
            MCAL_WARN("do fitting, no memory for txpwr tab n\r\n");
            return 0;
        } 
        os_memset(gtxpwr_tab_n, 0, sizeof(TXPWR_ST)*WLAN_2_4_G_CHANNEL_NUM);
    }    
    if(gtxpwr_tab_n) {
        ...
        if(g_tabn_use_def == 1)  
        {
            TXPWR_PTR tab_ptr_temp = gtxpwr_tab_g;
            if(!gtxpwr_tab_g){
                MCAL_WARN("tabe g must be exist, when tabe n is not ready\r\n");
                os_free(gtxpwr_tab_n);
                gtxpwr_tab_n = NULL;
                return 0;
            }
            if((!tab_ptr_temp[0].valid)||(!tab_ptr_temp[6].valid)||(!tab_ptr_temp[12].valid)) {
                MCAL_WARN("txpwr table g ch1 ch7 ch13 unused\r\n");
                os_free(gtxpwr_tab_n);
                gtxpwr_tab_n = NULL;
            return 0;
        }

            MCAL_WARN("txpwr table for n ch1 ch7 ch13 unused, use tabe g for instead\r\n");
            os_memcpy(&tab_ptr[0], &tab_ptr_temp[0], sizeof(TXPWR_ST));
            os_memcpy(&tab_ptr[6], &tab_ptr_temp[6], sizeof(TXPWR_ST));
            os_memcpy(&tab_ptr[12], &tab_ptr_temp[12], sizeof(TXPWR_ST));
            manual_cal_adjust_fitting(&tab_ptr[0], MOD_DIST_G_BW_N_LOW);
            manual_cal_adjust_fitting(&tab_ptr[6], MOD_DIST_G_BW_N_LOW);
            manual_cal_adjust_fitting(&tab_ptr[12], MOD_DIST_G_BW_N_HIGH);
        }
...
int manual_cal_load_txpwr_tab_flash(void)
{
    ...
    if(is_ready_flash & TXPWR_TAB_N_RD) {
        ...
        if(addr) {
            gtxpwr_tab_n = (TXPWR_PTR)os_malloc(sizeof(TXPWR_ST)*WLAN_2_4_G_CHANNEL_NUM); 
            if(!gtxpwr_tab_n) {
                MCAL_FATAL("no memory for txpwr tab n\r\n");
                return -1;
        }
            ddev_read(flash_handle, (char *)&head, sizeof(TXPWR_ELEM_ST), addr);
...
int manual_cal_load_default_txpwr_tab(void)
{
    #if TXPWR_DEFAULT_TAB
    if(!gtxpwr_tab_b) {
        gtxpwr_tab_b = (TXPWR_PTR)os_malloc(sizeof(TXPWR_ST)*WLAN_2_4_G_CHANNEL_NUM); 
    ...
    if(!gtxpwr_tab_n) {
        ...
        os_memcpy(gtxpwr_tab_n, gtxpwr_tab_def_n, sizeof(gtxpwr_tab_def_n));
        g_tabn_use_def = 1;
        MCAL_WARN("Load default txpwr for n:%p\r\n", gtxpwr_tab_def_n);
    }
    #endif
}
...
int manual_cal_save_txpwr_tab_flash(void)
{
    ...
    len = sizeof(TAG_TXPWR_ST) + sizeof(TAG_ENABLE_ST) + 3*sizeof(TAG_TXPWR_TAB_ST);
