...
static void default_Command(char *pcWriteBuffer, int xWriteBufferLen,int argc, char **argv)
{
...
static const struct cli_command user_clis[] = {
  {"micodebug", "micodebug on/off", micodebug_Command},
  	{"easylink", "easylink", easylink_Command},
...
int cli_init(void)
{
  ...
  memset(pCli, 0, sizeof(struct cli_st));
...
{
    uint32_t rx_size = 1;
  if (hal_uart_recv(CLI_UART, inbuf, &rx_size, MICO_WAIT_FOREVER) == 0)
    return 1;
  else
    return 0;
}
