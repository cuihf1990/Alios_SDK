...
echo -n "installing gcc-xtensa-esp8266 toolchain ..."
XTENSA_TOOLCHAIN_PATH=${OBJ_DIR}/build/compiler/gcc-xtensa-esp8266
if [ ! -d ${XTENSA_TOOLCHAIN_PATH} ]; then
    mkdir -p ${XTENSA_TOOLCHAIN_PATH}
fi
cd ${XTENSA_TOOLCHAIN_PATH}
...
if [ $? -ne 0 ]; then
    echo -e "\nerror: download gcc-xtensa-esp8266 toolchain faield\n"
    exit 1
fi
tar zxf *xtensa-lx106-elf.tar.gz
if [ $? -ne 0 ]; then
    echo -e "\nerror: extract gcc-xtensa-esp8266 toolchain faield\n"
    exit 1
fi
rm -f *xtensa-lx106-elf.tar.gz
mv *xtensa-lx106-elf ${HOST_OS}
exist=`cat ~/${BASHRC_FILE} | grep gcc-xtensa-esp8266`
if [ "${exist}" = "" ]; then
    echo "export PATH=\"${XTENSA_TOOLCHAIN_PATH}/${HOST_OS}/bin:\$PATH\"" >> ~/${BASHRC_FILE}
fi
echo "done"
